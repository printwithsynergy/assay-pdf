...
def ingest(
    xlsx: Annotated[
        Path | None,
        typer.Option("--xlsx", help="Path to GWG 2022 XLSX (default: spec/gwg-2022-spec.xlsx)."),
    ] = None,
    output: Annotated[
        Path | None,
        typer.Option(
            "--output",
            help="Where to write requirement-ids.json (default: spec/requirement-ids.json).",
        ),
    ] = None,
) -> None:
...
def fetch(
    force: Annotated[
        bool, typer.Option("--force", help="Re-download even if files exist with valid checksums.")
    ] = False,
) -> None:
...
def generate(
    only_rule: Annotated[
        str | None,
        typer.Option("--only-rule", help="Generate only files for one rule ID (e.g. R0014)."),
    ] = None,
    only_variant: Annotated[
        str | None,
        typer.Option("--only-variant", help="Generate only files for one variant kebab."),
    ] = None,
...
def benchmark(
    ...
    profile: Annotated[
        str | None,
        typer.Option("--profile", help="GWG 2022 variant kebab name (omit to run all variants)."),
    ] = None,
) -> None:
...
def validate(
    schema_only: Annotated[
        bool,
        typer.Option(
            "--schema-only", help="Skip verapdf; just validate manifests against schemas."
        ),
    ] = False,
) -> None:
